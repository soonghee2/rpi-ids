...
bool ClockSkewDetector::detectAnomaly(double error, uint32_t can_id) {
    ...
    if (m_detect_cnt > 5) {
        ...
        printf("[Masquerade] [%03x] [High] 연속 5번 이상 1.5 이상의 범위 밖으로 벗어났습니다. 지난 패킷의 meanError은 %.6f으로 정상 %03x의 %.6f보다 %.6f만큼 더 오차가 발생했습니다.", can_id, last_meanError, can_id, meanError, last_meanError-meanError);
