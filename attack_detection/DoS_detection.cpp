...
bool check_DoS(const EnqueuedCANMsg& dequeuedMsg, bool non_periodic) {
    ...
    if ((non_periodic && time_diff < Non_PERIODIC_DoS_TIME_THRESHOLD_MS) || (!non_periodic && time_diff < DoS_TIME_THRESHOLD_MS)) {
	    ...
	    //updateReasonMsg(dequeuedMsg.can_id, "DoS", "High", "5번 이상 %d개 패킷이 5ms이내로 빠르게 동일한 페이로드로 수신되었습니다.\n",stats.dos_count);
    ...
    if(memcmp(stats.dos_payload, dequeuedMsg.data, sizeof(dequeuedMsg.data)) == 0){
	 //updateReasonMsg(dequeuedMsg.can_id, "DoS", "High", "5ms이내로 빠르게 들어오지는 않았찌만 이전까지의 DoS Attack으로 판명된 페이로드로 비정상 주기로 수신되었습니다.");
